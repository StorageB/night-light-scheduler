...
export default class Prefs extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        /* Show banner if GNOME Night Light setting is off */
        ...
        const drawingArea = createScheduleGraph(
            schedule,
            clockFormat,
            MAX_TEMP,
            MIN_TEMP,
        ...
        function buildScheduleUI() {
            ...
            for (let entry of schedule) {
                ...
                const timeLabel = new Gtk.Label({
                    label:
                        clockFormat === "24h"
                            ? formatTime24h(totalMinutes)
                            : formatTimeAMPM(totalMinutes),
                ...
                function adjustTime(deltaMinutes) {
                    ...
                    if (deltaMinutes > 0) {
                        minutes =
                            Math.floor(minutes / timeIncrement) * timeIncrement;
                        minutes += timeIncrement;
                    } else {
                        minutes =
                            Math.ceil(minutes / timeIncrement) * timeIncrement;
                        minutes -= timeIncrement;
                    }
                    ...
                    timeLabel.set_label(
                        clockFormat === "24h"
                            ? formatTime24h(minutes)
                            : formatTimeAMPM(minutes),
                    );
                ...
                insertButton.connect("clicked", () => {
                    ...
                    let newMinutes = Math.floor(
                        (currentMinutes + nextMinutes) / 2,
                    );
                    newMinutes =
                        Math.round(newMinutes / timeIncrement) * timeIncrement;

                    if (nextMinutes - currentMinutes <= timeIncrement) return;

                    if (newMinutes <= currentMinutes)
                        newMinutes = currentMinutes + timeIncrement;
                    if (newMinutes >= nextMinutes)
                        newMinutes = nextMinutes - timeIncrement;
            ...
            drawingArea.setHoverCallback((index) => {
                ...
                if (index >= 0 && rows[index])
                    rows[index].add_css_class("row-hover-highlight");
            });
        ...
        const configRow = new Adw.ActionRow({
            subtitle: _(
                "Enable Night Light in GNOME Settings\n" +
                    ...
                    "•    Set Times so Night Light is always active (midnight to midnight)",
            ),
            activatable: false,
        });
