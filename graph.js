...
export function createScheduleGraph(schedule, clockFormat, max_temp, min_temp) {
    ...
    function tempToY(temp, height) {
        const ratio = (temp - chart_min_temp) / (max_temp - chart_min_temp);
    ...
    drawingArea.set_draw_func((area, cr, width, height) => {
        ...
        gradient.addColorStopRGBA(1, 1.0, 0.45, 0.1, 0.3);
        gradient.addColorStopRGBA(0.5, 1.0, 0.8, 0.4, 0.2);
        ...
        for (let h = 0; h <= 24; h += 6) {
            ...
            drawText(axisLabel, x, marginTop + graphHeight + 4);
        }
